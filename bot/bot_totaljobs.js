...
async function ensureLoggedIn(page) {
  ...
  console.log('  [Totaljobs Agent] Session active');
}

// ── Phase 1: Search & queue ───────────────────────────────────────────────
async function phase1_searchAndQueue(context, page) {
  console.log('\n══════════════════════════════════════════════════════');
  console.log('  [Totaljobs Agent] Phase 1 — Searching for jobs');
  console.log('══════════════════════════════════════════════════════');

  // Warm up on the homepage first — cold-jumping to a search URL triggers Akamai
  console.log('  [Totaljobs Agent] Warming up on homepage...');
  ...
  for (const searchTerm of cfg.JOB_SEARCHES) {
    console.log(`\n  [Totaljobs Agent] Searching: "${searchTerm}"`);
    ...
    try {
      ...
      console.error(`  [Totaljobs Agent] Failed to load search: ${err.message}`);
      continue;
    }
    ...
    if (!jobLinks.length) {
      console.log(`  [Totaljobs Agent] No results for "${searchTerm}"`);
      continue;
    }

    console.log(`  [Totaljobs Agent] Found ${jobLinks.length} listings`);

    for (const link of jobLinks.slice(0, cfg.MAX_JOBS_PER_SEARCH)) {
      const jobId = `tj_${link.url.replace(/[^a-z0-9]/gi, '_').slice(-40)}`;

      if (queue.has(jobId)) { console.log(`  [Totaljobs Agent] Already queued: ${link.title}`); continue; }
      if (queue.wasApplied(jobId)) { console.log(`  [Totaljobs Agent] Already applied — skipping: ${link.title}`); continue; }
      if (!isRelevantTitle(link.title)) { console.log(`  [Totaljobs Agent] Title filter — skipping: ${link.title}`); continue; }
      ...
      try {
        ...
        if (isBlockedCompany(company)) {
          console.log(`  [Totaljobs Agent] Company blocked — skipping: ${link.title} @ ${company}`);
          continue;
        }


        if (queue.hasCanonical(link.title, company)) {
          console.log(`  [Totaljobs Agent] Duplicate (cross-site) — skipping: ${link.title} @ ${company}`);
          continue;
        }
        ...
        if (applyHref && !applyHref.includes('totaljobs.com') && /^https?:\/\//.test(applyHref)) {
          console.log(`  [Totaljobs Agent] External site — skipping: ${link.title}`);
        ...
        if (!description || description.trim().split(/\s+/).length < 80) {
          console.log(`  [Totaljobs Agent] Short/missing JD — skipping: ${link.title}`);
        ...
        if (cfg.isTrainingCourseJD(description, link.title)) {
          console.log(`  [Totaljobs Agent] Training course — skipping: ${link.title}`);
        ...
        if (cfg.APPLICANT.seekSponsorship && !(await sponsorship.offersSponsorship(description))) {
          console.log(`  [Totaljobs Agent] No sponsorship — skipping: ${link.title}`);
        ...
        if (!cfg.WORK_TYPE_PRIORITY.includes(workType)) {
          console.log(`  [Totaljobs Agent] Work type "${workType}" not wanted — skipping: ${link.title}`);
        ...
        if (!salary.isAcceptable(description, cfg.APPLICANT.salaryExpectation)) {
          console.log(`  [Totaljobs Agent] Below salary expectation — skipping: ${link.title}`);
        ...
        console.log(`  [Totaljobs Agent] → Queued for Scorer: ${link.title} @ ${company} [${workType}]`);
      } catch (err) {
        console.error(`  [Totaljobs Agent] Error on "${link.title}": ${err.message}`);
      }
  ...
  console.log(`\n  [Totaljobs Agent] Phase 1 complete. ${pending} Totaljobs job(s) queued for Scorer.`);
  return page;
}
...
async function phase2_applyReadyCVs(page) {
  console.log('\n══════════════════════════════════════════════════════');
  console.log('  [Totaljobs Agent] Phase 2 — Waiting for Scorer agent...');
  console.log('══════════════════════════════════════════════════════');

  const retried = queue.requeueFailed('totaljobs');
  if (retried > 0) console.log(`  [Totaljobs Agent] Requeueing ${retried} previously-failed job(s) for retry.`);
  ...
  while (true) {
    ...
    for (const job of readyJobs) {
      ...
      if (appliedToday >= cfg.MAX_APPLICATIONS_PER_DAY) {
        console.log(`  [Totaljobs Agent] Daily limit reached (${appliedToday}/${cfg.MAX_APPLICATIONS_PER_DAY}) — pausing until tomorrow`);
        return;
      }
      ...
      if (!isRelevantTitle(job.title)) {
        ...
        console.log(`  [Totaljobs Agent] Post-queue title filter — skipping: ${job.title}`);
        continue;
      }

      queue.update(job.jobId, { status: 'applying' });
      console.log(`\n  [Totaljobs Agent] Applying: ${job.title} @ ${job.company} [${job.workType || 'onsite'}] (CV: ${job.cvName}, Score: ${job.cvScore}%)`);
      ...
      try {
        ...
        if (!applyBtn) {
          ...
          console.log(`  [Totaljobs Agent] Apply button gone — skipping: ${job.title}`);
          continue;
        }
        ...
        if (success) {
          ...
          console.log(`  [Totaljobs Agent] ✓ Applied: ${job.title} @ ${job.company}`);
        } else {
          ...
          console.log(`  [Totaljobs Agent] No confirmation — marking failed: ${job.title}`);
        }
      } catch (err) {
        ...
        if (isPageIssue) {
          ...
          console.log(`  [Totaljobs Agent] Page issue — skipping: ${job.title} (${err.message})`);
        } else {
          ...
          console.error(`  [Totaljobs Agent] Error applying to "${job.title}": ${err.message}`);
        }
      }

      const pause = 8000 + Math.random() * 7000;
      console.log(`  [Totaljobs Agent] Pausing ${Math.round(pause / 1000)}s before next application...`);
      await DELAY(pause);
    }
    ...
    if (readyJobs.length > 0) {
      ...
      console.log(`  [Totaljobs Agent] Waiting for Scorer agent... (${pendingJobs} Totaljobs job(s) in progress)`);
    } else {
      idleCount++;
      console.log(`  [Totaljobs Agent] Idle ${idleCount}/${MAX_IDLE} — no pending or ready Totaljobs jobs`);
    }
...
async function main() {
  ...
  console.log('═══════════════════════════════════════════════════════');
  console.log('  Totaljobs Agent — Starting (continuous mode)');
  console.log('═══════════════════════════════════════════════════════');

  const stuckApplying = queue.getByStatus('applying').filter(j => j.source === 'totaljobs');
  if (stuckApplying.length > 0) {
    console.log(`  [Totaljobs Agent] Recovering ${stuckApplying.length} interrupted job(s) → cv_ready`);
  ...
  while (true) {
    ...
    logger.printSummary();
    console.log('\n  [Totaljobs Agent] Cycle complete. Waiting 1 min before next search...');
    await DELAY(60 * 1000);
  }
}
